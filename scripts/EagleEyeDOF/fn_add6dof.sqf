...
params ["_unit",["_searchDist",DOF_scanDist],["_fov", 40]];

if (isNil "_unit" || isNull _unit) exitWith {diag_log text "[6DOF] [add6dof] Invalid unit selected. Exiting fn_add6dof.sqf"};

if (_unit getVariable ["XK_enable6dof", false]) exitWith {diag_log text format ["[6DOF] [add6dof] %1 %2 is already has 6DOF enabled. exiting fn_add6dof.sqf.", name _unit, getPosATL _unit]};

_unit setVariable ["XK_enable6dof", true, true];
...
private _id = [
    {
        ...
        if !(_unit getVariable ["XK_enable6dof", false]) exitWith {
        ...
        //Mark targets in _searchDist if can linetrace to eyes or abdomen, and is within _fov
        /* private _targets = (_unit nearEntities ["CAManBase", _searchDist]) select {
            (_x != _unit) && 
        ...
        _unit setVariable ["XK_6dofList", _targetsFilter, true];
    },
    1,
    [_unit, _searchDist, _fov]
] call CBA_fnc_addPerFrameHandler;

missionNamespace setVariable ["XK_6dofPFH", _id];